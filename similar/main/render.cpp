...
#ifndef NDEBUG
namespace {
    static void draw_outline(const g3_draw_line_context &context, const unsigned nverts,
                             cg3s_point *const *const pointlist) {
...
namespace dsx {
	...
	if (eclip_num == ECLIP_NUM_FORCE_FIELD || eclip_num == ECLIP_NUM_FORCE_FIELD2)
        ...
        static void render_face(grs_canvas &canvas, const shared_segment &segp, const sidenum_t sidenum,
                                const unsigned nv, const std::array<vertnum_t, 4> &vp, const texture1_value tmap1,
                                const texture2_value tmap2, std::array<g3s_uvl, 4> uvl_copy,
                                const wall_is_doorway_result wid_flags) {
            auto &LevelUniqueControlCenterState = LevelUniqueObjectState.ControlCenterState;
	...
	const auto Seismic_tremor_magnitude = LevelUniqueSeismicState.Seismic_tremor_magnitude;
            ...
            for (auto &&[dli, uvli, vpi]: zip(std::span(dyn_light).first(nv), uvl_copy, vp)) {
                ...
                if (need_flashing_lights) //make lights flash
                {
                    ...
                    dli.r = (!Seismic_tremor_magnitude && PlayerCfg.DynLightColor)
                                ? fixmul(std::max(static_cast<double>(flash_scale), f0_5 * 1.5), uvli.l)
                                // let the mine glow red a little
                                : dli.g;
                }

                // add light color (version2?)
                add_light_and_saturate(dli.r, Dlvpi.r);
                add_light_and_saturate(dli.g, Dlvpi.g);
                add_light_and_saturate(dli.b, Dlvpi.b);
                if (PlayerCfg.AlphaEffects)
                // due to additive blending, transparent sprites will become invisible in font of white surfaces (lamps). Fix that with a little desaturation
                {
                    dli.r *= .93;
                    dli.g *= .93;
                    dli.b *= .93;
            ...
            bool alpha = false;
            if (PlayerCfg.AlphaBlendEClips && is_alphablend_eclip(TmapInfo[get_texture_index(tmap1)].eclip_num)) {
                // set nice transparency/blending for some special effects (if we do more, we should maybe use switch here)
                alpha = true;
                gr_settransblend(canvas, GR_FADE_OFF, gr_blend::additive_c);
            }
	...
	if (Render_only_bottom && sidenum == sidenum_t::WBOTTOM)
		g3_draw_tmap(canvas, nv, pointlist, uvl_copy, dyn_light, GameBitmaps[Textures[Bottom_bitmap_num]], draw_tmap);
            ...
            if (alpha)
                gr_settransblend(canvas, GR_FADE_OFF, gr_blend::normal);
            // revert any transparency / blending setting back to normal
...
namespace {
    ...
    static void check_face(grs_canvas &canvas, const vmsegidx_t segnum, const sidenum_t sidenum, const unsigned facenum,
                           const unsigned nv, const std::array<vertnum_t, 4> &vp, const texture1_value tmap1,
                           const texture2_value tmap2, const std::array<g3s_uvl, 4> &uvl_copy) {
#if DXX_USE_EDITOR
	if (_search_mode) {
		std::array<g3s_lrgb, 4> dyn_light{};
		std::array<cg3s_point *, 4> pointlist;
    ...
    static inline void check_render_face(grs_canvas &canvas, std::index_sequence<N...>, const vcsegptridx_t segnum,
                                         const sidenum_t sidenum, const unsigned facenum,
                                         const std::array<vertnum_t, 4> &ovp, const texture1_value tmap1,
                                         const texture2_value tmap2, const std::array<uvl, 4> &uvlp,
                                         const wall_is_doorway_result wid_flags, const std::size_t nv) {
    ...
    static inline void check_render_face(grs_canvas &canvas, std::index_sequence<N0, N1, N2, N3> is,
                                         const vcsegptridx_t segnum, const sidenum_t sidenum, const unsigned facenum,
                                         const std::array<vertnum_t, 4> &vp, const texture1_value tmap1,
                                         const texture2_value tmap2, const std::array<uvl, 4> &uvlp,
                                         const wall_is_doorway_result wid_flags) {
    ...
    static inline void check_render_face(grs_canvas &canvas, std::index_sequence<N0, N1, N2>,
                                         const vcsegptridx_t segnum, const sidenum_t sidenum, const unsigned facenum,
                                         const std::array<vertnum_t, 4> &vp, const texture1_value tmap1,
                                         const texture2_value tmap2, const std::array<uvl, 4> &uvlp,
                                         const wall_is_doorway_result wid_flags) {
        check_render_face(canvas, std::index_sequence<N0, N1, N2, 3>(), segnum, sidenum, facenum, vp, tmap1, tmap2,
                          uvlp, wid_flags, 3);
    }
...
namespace dsx {
    namespace {
        static void render_side(fvcvertptr &vcvertptr, grs_canvas &canvas, const vcsegptridx_t segp,
                                const sidenum_t sidenum, const wall_is_doorway_result wid_flags,
                                const vms_vector &Viewer_eye) {
            fix min_dot, max_dot;

            if (!(wid_flags & WALL_IS_DOORWAY_FLAG::render))
                //if (WALL_IS_DOORWAY(segp, sidenum) == wall_is_doorway_result::no_wall)
                return;
            ...
            if (sside.type == side_type::quad) {
                if (v_dot_n0 >= 0) {
                    check_render_face(canvas, is_quad, segp, sidenum, 0, vertnum_list, uside.tmap_num, uside.tmap_num2,
                                      uside.uvls, wid_flags);
                }
            } else {
                ...
                if (DETRIANGULATION && (min_dot + F1_0 / 256 > max_dot || (
                                            Viewer->segnum != segp && min_dot > Tulate_min_dot && max_dot < min_dot *
                                            2)) &&
                    ...
                    if (sside.type == side_type::tri_02) {
                        if (v_dot_n0 >= 0) {
                            check_render_face(canvas, std::index_sequence<0, 1, 2>(), segp, sidenum, 0, vertnum_list,
                                              uside.tmap_num, uside.tmap_num2, uside.uvls, wid_flags);
                        }

                        if (v_dot_n1 >= 0) {
                            // want to render from vertices 0, 2, 3 on side
                            check_render_face(canvas, std::index_sequence<0, 2, 3>(), segp, sidenum, 1, vertnum_list,
                                              uside.tmap_num, uside.tmap_num2, uside.uvls, wid_flags);
                        }
                    } else if (sside.type == side_type::tri_13) {
                        if (v_dot_n1 >= 0) {
                            // rendering 1,2,3, so just skip 0
                            check_render_face(canvas, std::index_sequence<1, 2, 3>(), segp, sidenum, 1, vertnum_list,
                                              uside.tmap_num, uside.tmap_num2, uside.uvls, wid_flags);
                        }

                        if (v_dot_n0 >= 0) {
                            // want to render from vertices 0,1,3
                            check_render_face(canvas, std::index_sequence<0, 1, 3>(), segp, sidenum, 0, vertnum_list,
                                              uside.tmap_num, uside.tmap_num2, uside.uvls, wid_flags);
                        }
                    } else
...
static void render_object_search(grs_canvas &canvas, const d_level_unique_light_state &LevelUniqueLightState, const vmobjptridx_t obj)
{
	...
	if (changed) {
        ...
        static void do_render_object(grs_canvas &canvas, const d_level_unique_light_state &LevelUniqueLightState,
                                     const vmobjptridx_t obj, window_rendered_data &window) {
            int count{0};

#ifndef NDEBUG
            if (object_rendered[obj]) {
	  ...
	  if ((DemoDoingLeft==6 || DemoDoingRight==6) && obj->type==OBJ_PLAYER)
		{
            ...
            if (obj->type == OBJ_ROBOT && obj->ctype.ai_info.ail.player_awareness_type ==
                player_awareness_type_t::PA_NONE && (obj.get_unchecked_index() & 3) == (d_tick_count & 3))
...
g3s_codes rotate_list(fvcvertptr &vcvertptr, const std::span<const vertnum_t> pointnumlist) {
    ...
    for (const auto pnum: pointnumlist) {
        ...
        if (pnt.p3_last_generation != current_generation) {
            ...
            g3_rotate_point(pnt, likely(!cheats_acid)
                                     ? v
                                     : (
                                         vertex{
...
namespace dsx {
    namespace {
        static void render_segment(fvcvertptr &vcvertptr, fvcwallptr &vcwallptr, const vms_vector &Viewer_eye,
                                   grs_canvas &canvas, const vcsegptridx_t seg) {
		...
		if (Viewer->type != OBJ_ROBOT)
                ...
                for (const auto sn: MAX_SIDES_PER_SEGMENT)
                    render_side(vcvertptr, canvas, seg, sn, WALL_IS_DOORWAY(GameBitmaps, Textures, vcwallptr, seg, sn),
                                Viewer_eye);
            }
...
namespace dcx {
    ...
    namespace {
        ...
        constexpr std::array<
            ...
            8> Edge_to_sides = {
            {
                {
                    {
                        {{side_none, side_none}}, {{sidenum_t::WRIGHT, sidenum_t::WFRONT}}, {{side_none, side_none}},
                        {{sidenum_t::WTOP, sidenum_t::WFRONT}}, {{sidenum_t::WTOP, sidenum_t::WRIGHT}},
                        {{side_none, side_none}}, {{side_none, side_none}}, {{side_none, side_none}}
                    }
                },
                {
                    {
                        {{sidenum_t::WRIGHT, sidenum_t::WFRONT}}, {{side_none, side_none}},
                        {{sidenum_t::WBOTTOM, sidenum_t::WFRONT}}, {{side_none, side_none}}, {{side_none, side_none}},
                        {{sidenum_t::WRIGHT, sidenum_t::WBOTTOM}}, {{side_none, side_none}}, {{side_none, side_none}}
                    }
                },
                {
                    {
                        {{side_none, side_none}}, {{sidenum_t::WBOTTOM, sidenum_t::WFRONT}}, {{side_none, side_none}},
                        {{sidenum_t::WLEFT, sidenum_t::WFRONT}}, {{side_none, side_none}}, {{side_none, side_none}},
                        {{sidenum_t::WLEFT, sidenum_t::WBOTTOM}}, {{side_none, side_none}}
                    }
                },
                {
                    {
                        {{sidenum_t::WTOP, sidenum_t::WFRONT}}, {{side_none, side_none}},
                        {{sidenum_t::WLEFT, sidenum_t::WFRONT}}, {{side_none, side_none}}, {{side_none, side_none}},
                        {{side_none, side_none}}, {{side_none, side_none}}, {{sidenum_t::WLEFT, sidenum_t::WTOP}}
                    }
                },
                {
                    {
                        {{sidenum_t::WTOP, sidenum_t::WRIGHT}}, {{side_none, side_none}}, {{side_none, side_none}},
                        {{side_none, side_none}}, {{side_none, side_none}}, {{sidenum_t::WRIGHT, sidenum_t::WBACK}},
                        {{side_none, side_none}}, {{sidenum_t::WTOP, sidenum_t::WBACK}}
                    }
                },
                {
                    {
                        {{side_none, side_none}}, {{sidenum_t::WRIGHT, sidenum_t::WBOTTOM}}, {{side_none, side_none}},
                        {{side_none, side_none}}, {{sidenum_t::WRIGHT, sidenum_t::WBACK}}, {{side_none, side_none}},
                        {{sidenum_t::WBOTTOM, sidenum_t::WBACK}}, {{side_none, side_none}}
                    }
                },
                {
                    {
                        {{side_none, side_none}}, {{side_none, side_none}}, {{sidenum_t::WLEFT, sidenum_t::WBOTTOM}},
                        {{side_none, side_none}}, {{side_none, side_none}}, {{sidenum_t::WBOTTOM, sidenum_t::WBACK}},
                        {{side_none, side_none}}, {{sidenum_t::WLEFT, sidenum_t::WBACK}}
                    }
                },
                {
                    {
                        {{side_none, side_none}}, {{side_none, side_none}}, {{side_none, side_none}},
                        {{sidenum_t::WLEFT, sidenum_t::WTOP}}, {{sidenum_t::WTOP, sidenum_t::WBACK}},
                        {{side_none, side_none}}, {{sidenum_t::WLEFT, sidenum_t::WBACK}}, {{side_none, side_none}}
                    }
                },
            }
        };
        ...
        static std::optional<sidenum_t> find_seg_side(const shared_segment &seg, const std::array<vertnum_t, 2> &verts,
                                                      const sidenum_t notside) {
        ...
        [[nodiscard]]
        static bool compare_child(fvcvertptr &vcvertptr, const vms_vector &Viewer_eye, const shared_segment &seg,
                                  const shared_segment &cseg, const sidenum_t edgeside) {
            const auto &cside = cseg.sides[edgeside];
            const auto &sv = Side_to_verts[edgeside][cside.type == side_type::tri_13
                                                         ? side_relative_vertnum::_1
                                                         : side_relative_vertnum::_0];
        ...
        [[nodiscard]]
        static bool compare_children(fvcvertptr &vcvertptr, const vms_vector &Viewer_eye, const vcsegptridx_t seg,
                                     const sidenum_t s0, const sidenum_t s1) {
        ...
        static void sort_seg_children(fvcvertptr &vcvertptr, const vms_vector &Viewer_eye, const vcsegptridx_t seg,
                                      const std::ranges::subrange<sort_child_array_t::iterator> &r) {
        ...
        public:
            ...
            render_compare_context_t(fvcobjptr &vcobjptr, const vms_vector &Viewer_eye,
                                     const render_state_t::per_segment_state_t &segstate) {
	...
	{		//same position
        ...
        static void sort_segment_object_list(fvcobjptr &vcobjptr, const vms_vector &Viewer_eye,
                                             render_state_t::per_segment_state_t &segstate) {
...
namespace dsx {
    namespace {
        static void build_object_lists(object_array &Objects, fvcsegptr &vcsegptr, const vms_vector &Viewer_eye,
            ...
            range_for(const unsigned nn, xrange(N_render_segs)) {
				...
				if (obj->type != OBJ_CNTRLCEN && !(obj->type==OBJ_ROBOT && get_robot_id(obj) == robot_id::special_reactor))		//don't migrate controlcen
					...
					did_migrate = 0;
#endif
                            if (const auto sidemask = get_seg_masks(vcvertptr, obj->pos, vcsegptr(new_segnum),
                                                                    obj->size).sidemask; sidemask != sidemask_t{}) {
                                for (const auto sn: MAX_SIDES_PER_SEGMENT) {
								...
								const cscusegment &&seg = vcsegptr(new_segnum);
                                        ...
                                        if (WALL_IS_DOORWAY(GameBitmaps, Textures, vcwallptr, seg, sn) &
                                            WALL_IS_DOORWAY_FLAG::fly) {
...
namespace dsx {
    ...
    namespace {
        ...
        static void build_segment_list(render_state_t &rstate, const vms_vector &Viewer_eye,
                                       visited_twobit_array_t &visited, unsigned &first_terminal_seg,
                                       const vcsegidx_t start_seg_num) {
            auto &LevelSharedVertexState = LevelSharedSegmentState.get_vertex_state();
            auto &Vertices = LevelSharedVertexState.get_vertices();
            int lcnt, scnt, ecnt;
            int l;
                        ...
                        const auto ch = seg->shared_segment::children[siden]; {
                            {
                                ...
                                if (no_proj_flag || (
                                        codes_and_3d == clipping_code::None && codes_and_2d == clipping_code::None)) {
    ...
    //renders onto current canvas
    void render_mine(grs_canvas &canvas, const vms_vector &Viewer_eye, const vcsegidx_t start_seg_num,
                     const fix eye_offset, window_rendered_data &window) {
        auto &LevelSharedVertexState = LevelSharedSegmentState.get_vertex_state();
        auto &Objects = LevelUniqueObjectState.Objects;
        auto &Vertices = LevelSharedVertexState.get_vertices();
	...
	//else
        ...
        build_segment_list(rstate, Viewer_eye, visited, first_terminal_seg, start_seg_num);
        //fills in Render_list & N_render_segs
        ...
        if (!_search_mode && Clear_window == 2) {
            if (first_terminal_seg < rstate.N_render_segs) {
                ...
                range_for(const auto segnum,
                          partial_const_range(rstate.Render_list, first_terminal_seg, rstate.N_render_segs)) {
                    if (segnum != segment_none) {
                ...
                // render segment
                {
                    ...
                    if (rotate_list(vcvertptr, seg->verts).uand == clipping_code::None) {
                        ...
                        for (const auto sn: MAX_SIDES_PER_SEGMENT) {
                            ...
                            if (wid == wall_is_doorway_result::transparent_wall || wid ==
                                wall_is_doorway_result::transillusory_wall
#if DXX_BUILD_DESCENT == 2
							|| (wid & WALL_IS_DOORWAY_FLAG::cloaked)
#endif
                            ) {
                                if (PlayerCfg.AlphaBlendEClips && is_alphablend_eclip(
                                        TmapInfo[get_texture_index(seg->unique_segment::sides[sn].tmap_num)].eclip_num))
                                ...
                                // prevent ugly outlines if an object (which is rendered later) is shown behind a grate, door, etc. if texture filtering is enabled. These sides are rendered later again with normal AlphaFunc
            ...
            if (segnum != segment_none && (_search_mode || visited[segnum] != 3)) {
                    ...
                    continue;
                {
                    ...
                    range_for(auto &v, srsm.objects) {
                        do_render_object(canvas, LevelUniqueLightState, vmobjptridx(v.objnum), window);
    ...
    //-------------- Renders a hostage --------------------------------------------
    void draw_hostage(const d_vclip_array &Vclip, grs_canvas &canvas,
                      const d_level_unique_light_state &LevelUniqueLightState, const vmobjptridx_t obj) {
