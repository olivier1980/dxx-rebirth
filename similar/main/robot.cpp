...
namespace dsx {
    ...
    //fills in gun_point
    void calc_gun_point(const robot_info &r, vms_vector &gun_point, const object_base &obj,
                        const robot_gun_number entry_gun_num) {
    ...
    std::ranges::subrange<const jointpos *> robot_get_anim_state(const d_robot_info_array &robot_info,
                                                                 const std::array<jointpos, MAX_ROBOT_JOINTS> &
                                                                 robot_joints, const robot_id robot_type,
                                                                 const robot_gun_animation_index gun_num,
                                                                 const robot_animation_state state) {
    ...
    //be filled in.
    void robot_set_angles(robot_info &r, polymodel &pm,
                          enumerated_array<std::array<vms_angvec, MAX_SUBMODELS>, N_ANIM_STATES, robot_animation_state>
                          &angs) {
        auto &Robot_joints = LevelSharedRobotJointState.Robot_joints;
        ...
        for (const std::size_t bound{std::min(gun_nums.size(), pm.submodel_parents.size())}; auto [g, entry_m]:
             enumerate(partial_const_range(r.gun_submodels, r.n_guns))) {
            auto m = entry_m;
            while (m != 0 && m < bound) {
...
namespace dsx {
    imobjptridx_t robot_create(const d_robot_info_array &Robot_info, const robot_id id, const vmsegptridx_t segnum,
                               const vms_vector &pos, const vms_matrix *const orient, const fix size,
                               const ai_behavior behavior, const imsegidx_t hide_segment) {
        const auto &&objp = obj_create(LevelUniqueObjectState, LevelSharedSegmentState, LevelUniqueSegmentState,
                                       OBJ_ROBOT, underlying_value(id), segnum, pos, orient, size,
                                       object::control_type::ai, object::movement_type::physics,
                                       render_type::RT_POLYOBJ);
        if (objp)
	...
	PHYSFSX_skipBytes<4 * (NDL * 2)>(fp);
        ...
        std::cout << "Model mass: " + std::to_string(ri.mass) << std::endl;
        std::cout << "Weapon type: " + std::to_string(ri.weapon_type) << std::endl << std::endl;
